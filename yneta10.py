...
class MyViT(nn.Module):
  def __init__(self, chw=(1, 28, 28), n_patches=7, n_blocks=2, hidden_d=10, n_heads=2,out_d = 100):
    ...
    self.mlpl2 = nn.Linear(out_d,self.hidden_d )
  ...
  def forward(self, images):
      ...
      reimage2 = depatchify(out2,self.n_patches)
...
def main():
    ...
    with open('atrain_combined10_data.pkl','rb') as f: traindata = pickle.load(f)
    with open('atrain_combined10_labels.pkl','rb') as f: trainlabels = pickle.load(f)
    
    with open('atest_combined10_data.pkl','rb') as f: testdata = pickle.load(f)
    with open('atest_combined10_labels.pkl','rb') as f: testlabels = pickle.load(f)
    
    traindata = traindata.reshape(250000,2,1,28,28)
    testdata = testdata.reshape(50000,2,1,28,28)
    ...
    # plt.imshow(x[0,0,0,:,:])
    # plt.show()
    # plt.imshow(torch.squeeze(x_hat[0,0,:,:].detach()))
    # plt.show()
    
    # plt.imshow(x2[0,0,:,:])
    # plt.show()
    # plt.imshow(torch.squeeze(x_hat2[0,0,:,:].detach()))
    # plt.show()
    ...
    #Test loop
    latent_save = np.zeros((10,49,100,5000))
    with torch.no_grad():
        test_loss = 0.0
        for i in range(0,10):
            idx = testlabels == i
        ...
        #plt.imshow(x2[0,0,:,:])
        #plt.show()
        #plt.imshow(torch.squeeze(x_hat2[0,0,:,:]))
        #plt.show()
        
        with open('ls_acombined1010ep.pkl','wb') as f: pickle.dump(latent_save, f)

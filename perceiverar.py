...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 23 14:07:59 2023
adapted from
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, max_seq_len, *, device):
        ...
        freqs = einsum("i , j -> i j", seq, self.inv_freq) #outerproduct
        return torch.cat((freqs, freqs), dim = -1)
    
def rotate_half(x):
    x = rearrange(x, "... (j d) -> ... j d", j = 2) #splitting one dimension into two dimensions by halfing the dimension
    x1, x2 = x.unbind(dim = -2)                     #unbinding along the added dimension
    return torch.cat((-x2, x1), dim = -1)           #concatenating the two tensors by rotating i.e. mulitplying with -1 and and switching x1 and x2 


def apply_rotary_pos_emb(pos, t):
    seq_len, rotate_dim = t.shape[-2], pos.shape[-1]
    pos = pos[..., -seq_len:, :]
    t, t_pass = t[..., :rotate_dim], t[..., rotate_dim:] #splitting into t and tpass 
    t = (t * pos.cos()) + (rotate_half(t) * pos.sin()) #applying sin and cosine 
...
class CausalAttention(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb = None):
        ...
        sim = einsum('b h i d, b h j d -> b h i j', q, k) #dot product

        i, j = sim.shape[-2:]
        causal_mask = torch.ones((i, j), device = x.device, dtype = torch.bool).triu(j - i + 1) #masks everything but 

...
with open('melspec2.pkl','rb') as f2: freq_images= pickle.load(f2)
...
class MyViT(nn.Module):
  def __init__(self, chw=(1, 225, 225), n_patches=15, n_blocks=2, hidden_d=10, n_heads=2,out_d = 100):
    # Super constructor
...
def main():
    ...
    my_dataloader = DataLoader(my_dataset,shuffle=True, batch_size=50)
    #test_set0 = test_set
    ...
    model = MyViT((1, 225, 225), n_patches=15, n_blocks=2, hidden_d = 10, n_heads=2, out_d=100).to(device)
    N_EPOCHS = 5
    LR = 0.001
    ...
    # Test loop
    latent_save = np.zeros((10,225,100,500))
    with torch.no_grad():
        test_loss = 0.0
        for i in range(0,10):
            idx = my_dataset[:][1] == i
        ...
        with open('ls_auds22.pkl','wb') as f: pickle.dump(latent_save, f)
